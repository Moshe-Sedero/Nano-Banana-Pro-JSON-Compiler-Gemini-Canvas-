...
Conflict Resolution: Visual badges and tooltips indicate when and why a specific field was auto-corrected.

Multi-JSON Batching: Generate sequences of prompts in a single session for batch processing.

Multi-Panel Mode: Option to append instructions for creating multi-panel collages from the generated batch.
...
SCHEMA: A declarative object defining all fields, data types (select, multi_select, boolean, text), options, and default values.

CONSTRAINTS: Defines rules for compatibility (e.g., Scene vs. Apparel) and strict logic dependencies.

usePromptEngine (Custom Hook): The "brain" of the application. It handles:

State synchronization.

Constraint validation pipeline (validateAndFix).
...
Note: Tailwind CSS is required for styling. Follow the Tailwind CSS Vite Guide to configure it.

Replace the contents of src/App.jsx with the code from this repository.
...
To modify logic rules (e.g., preventing "Swimwear" in a "Office" setting), edit the CONSTRAINTS constant.
